...
* **Smart Cache Protection:** Injects a custom `dfree.sh` script into Samba to report a fixed size of free space, preventing clients from uploading files larger than the local disk cache.

## Prerequisites

### Requirements
* A Telegram account and at least 1 Telegram bot token.
* A server or VM with a minimum of 35 GB storage (edit the script to set a different size).
...
```
> To customise the installation, edit the script before running it.
...
* **dfree.sh:** A script injected into Samba that forces the OS to report a fixed amount of free space to protect the local cache.
